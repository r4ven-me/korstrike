...
- **`cs-server`** — a [ReHLDS](https://github.com/rehlds/ReHLDS)-based dedicated
  server (a hardened, bug-fixed drop-in replacement for the stock HLDS
  engine), preloaded with the classic retail map pool, with
...
- **`panel`** — a small custom web UI for day-to-day admin: live player list,
  kick/ban, map switching, server password, ~50 gameplay/network/physics
  cvars, a raw RCON console, and an activity log.
...
its own Docker volume (`maps-data`), shared between `cs-server` and `panel`,
so maps land in normal Docker-managed storage rather than a host folder.
Three ways to add a map:
...
1. **Download it from the panel.** Map tab → "Download a map" → give it the
   ...
   straight into the shared volume — **playable immediately via
   `changelevel`, no restart needed** — and shows up in the map switcher
   right away.
2. **Drop files in directly.** Put the map's `.bsp` (and matching resource
   files) into `./maps/` on the host — this folder is still bind-mounted and
   synced into the shared volume on every container (re)start, and
   auto-added to the mapcycle at that point.
3. **Bulk pre-seed before first boot.** Copy `maps/maps-list.txt.example` to
   `maps/maps-list.txt` and add one `name=url` line per map, same as before;
   `cs-server` fetches anything listed there on startup (or via
   `docker compose exec cs-server /server/scripts/download-maps.sh`).
   Already-downloaded maps are skipped, so it's safe to re-run any time.
...
| Tab | What it does |
|---|---|
| Игроки (Players) | Live player list (name, SteamID, IP, ping, time, frags), updated over a WebSocket every ~2.5s. Kick or ban directly from a row. |
| Карта (Map) | Change the live map from a dropdown of all installed maps; download a new map from a URL directly into the shared maps volume, playable immediately. |
...
flag on a small shared `control-data` volume between each run — the panel
just sets/clears that flag, no RCON or Docker access needed either way. The
container itself keeps running either way; only the game process inside
stops.
...
build time, same as ReHLDS/ReUnion — see the comments in `server/Dockerfile`
...
- The panel talks to the game server purely over the network, using the
  **GoldSrc RCON protocol** (a small UDP challenge/response scheme — not the
  same protocol as Source-engine RCON). It polls `status` periodically for
  the player list and issues admin commands (`kick`, `banid`, `addip`,
  `changelevel`, cvar sets, etc.) the same way you would over `rcon` in a
  console.
- `RCON_PASSWORD` in `.env` is shared between both services — the server
  enforces it, the panel authenticates with it. It has no built-in default;
...
- The panel itself has a single admin account, seeded from `ADMIN_USER` /
  ...
  sessions. `SESSION_SECRET` is auto-generated and persisted to the
  `panel-data` volume if you don't set one.
...
- Both containers run as **non-root users**, with `cap_drop: ALL` and
  `no-new-privileges`, plus memory/CPU limits in `docker-compose.yml`.
...
- **Container restarts immediately / "RCON_PASSWORD is required"**: you
  haven't set `RCON_PASSWORD` (and/or `ADMIN_USER`/`ADMIN_PASSWORD` for the
  panel) in `.env`.
- **Panel shows no players / "not polled yet"**: check
  `docker compose logs panel` — usually means `RCON_PASSWORD` doesn't match
  between the two services, or the game server hasn't finished starting yet.
...
- **Custom map doesn't show up**: check `docker compose logs cs-server` for
